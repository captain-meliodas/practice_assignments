def obstacles(obs,start):
    for i in range(len(obs)):
        if start == obs[i][0]:
            return obstacles(obs,obs[i][1])
    return start
...
no_of_s_and_l = int(input())
obs = []
for i in range(no_of_s_and_l):
    start,end = [int(j) for j in input().split(" ")]
    obs.append((start,end))


no_dice_throws = int(input())
throws = input().split(" ")
player = 'A'
result = {'A':0,'B':0}
for throw in throws:
    throw = int(throw)
    if result[player] + throw <= 99:
        result[player] += throw
    if result[player] == 99:
        break
    result[player] = obstacles(obs,result[player])
